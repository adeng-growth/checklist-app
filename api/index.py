...
# -*- coding: utf-8 -*-
"""
中小企业增长自检清单 - Vercel部署版（api/index.py）
使用 Airtable 作为云数据库
...
from flask import session, redirect, render_template_string, send_file
from io import BytesIO

# Vercel Python Serverless: 当前文件所在目录
...
def _export():
    try:
        ...
        return jsonify({'success': False, 'message': '请先安装openpyxl'}), 500
    ...
    fname = f"自检清单数据_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
